...
## Part 1 — 先起一个 slug，生成草稿文件

根据对话内容确定一个 slug（kebab-case，2-5 词），然后调用 Bash 生成草稿文件（包含完整时间线，摘要为占位符）：

```bash
node ~/.claude/scripts/session-finalize.mjs --draft <slug>
```

脚本会输出文件路径，记住这个路径（后续 Part 需要用到）。

## Part 2 — 按需刷新记忆

判断你是否对本次对话的内容有完整的记忆。

**如果你不确定**（比如中间发生过 `/compact`、context 被压缩、或者 session 很长），用 Read 工具读取 Part 1 生成的草稿文件。文件里有完整的对话时间线，可以帮你恢复记忆。
...
把 Part 3 的摘要填入草稿文件，替换占位符：

```bash
cat <<'SESSION_EOF' | node ~/.claude/scripts/session-finalize.mjs --fill-summary <Part 1 的文件路径>
---
slug: <同 Part 3>
title: <同 Part 3>
summary: <同 Part 3>
...
Bash 执行完后，用一句话确认，包含文件路径。不要重复输出摘要。不要再调用其他工具。
